...
class AlgoTrader implements Flow.Subscriber<TradeEvent> {
	...
	public void onNext(TradeEvent event) {
		...
		System.err.println("[%s] AlgoTrader: %s".formatted(Thread.currentThread().getName(),event));		
		this.subscription.request(1);
	}
...
class SignalProcessor implements Flow.Subscriber<TradeEvent> {
	...
	@Override
	public void onNext(TradeEvent event) {
		System.err.println("[%s] SignalProcessor: %s".formatted(Thread.currentThread().getName(),event));		
		this.subscription.request(1);
	}
